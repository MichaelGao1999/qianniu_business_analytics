...
本项目为 **AgentOne 自包含技能包**，采用**分层流水线架构**：
...
┌─────────────────────────────────────────────────────────────┐
│                   编排层（Orchestration）                     │
│         qianniu_analytics_orchestrator.py                   │
│              │                                              │
│    ┌─────────┼─────────┐                                   │
│    ▼         ▼         ▼                                   │
│  预检 → 取数 → 报告 → 推送                                   │
...
### M01 认证管理（Auth Management）

**职责**：管理 JYCM OpenAPI 的 Cookie 生命周期（获取、缓存、刷新、验证）。

**核心文件**：
- `auth/jycm.json` — 凭证文件（AK/SK/Cookie）
- `auth.md` — 认证流程文档
...
### M02 淘系取数引擎（Fetch Engine）

**职责**：执行 JYCM OpenAPI A→G 接口链，获取生意参谋数据并下载 Excel。

**核心文件**：
- `scripts/jycm_fetch_sycm_shop.py` — 取数脚本
- `fetch-data.md` — 取数规则文档
- `fetch-data-api.md` — API 完整参考
...
**核心文件**：
- `SKILL.md` — 技能主入口
- `scripts/qianniu_analytics_orchestrator.py` — 编排脚本
