...
> 阶段二设计时，AI 必须扫描项目 `lessons-learned.md`，按标签过滤与本项目相关的经验，逐一回答以下检查项。
> 本清单确保"上次踩过的坑，这次不再踩"。
...
1. 读取 `lessons-learned.md` 的「技术经验」表格
...
> 以下模板条目可根据本项目实际情况增删。每条检查项对应 `lessons-learned.md` 中的一条或多条经验。
...
> 根据本项目特征，从 `lessons-learned.md` 中补充其他相关检查项。
