...
| **决策** | **B. 聚焦 Excel 驱动流** |
| **理由** | 1. SKILL.md 已明确技能形态为「本地 Excel 驱动版」，当前 90% 价值来自这条流<br>2. API 流涉及外部认证、Digest 刷新、真实 BASE_URL 配置，维护成本高，且无活跃需求<br>3. Excel 驱动流已能满足「单店/多店分析 → Markdown 报告 → 飞书/钉钉推送」完整闭环<br>4. API 流代码骨架已删除（2026-06-25 清理），A→G 接口链知识保留在 lessons-learned.md 中，未来有需求时需参照经验重构 |
| **后果** | 正面：资源集中，Excel 流在短期内达到生产级。负面：API 取数自动化能力暂时缺失，旧骨架已清理。 |
| **可逆性** | **低**。API 流代码已删除，重建需从 lessons-learned.md 中提取接口知识从头实现。 |

*新增决策时复制上方模板，填写后追加到文件末尾。*

## ADR-019：Node.js 环境隔离方案（nvm + 双 Node.js） [母库 @2026-05-30] [来源:vibe-coding-project-sop @2026-05-31]
...
## ADR-020：状态文档机制重构（待办清理 + 技术债务表格化） [母库 @2026-05-30] [来源:vibe-coding-project-sop @2026-05-31]
