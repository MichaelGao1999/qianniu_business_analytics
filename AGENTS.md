...
**触发词**：`日记`（去除标点后精确等于这两个字）

**防误触**：
- 消息精确匹配「日记」→ 执行日记提取
- 消息包含「日记」但还有其他内容 → 视为正常对话，不触发（追加自然语言日期时除外，如 `日记 前天`）

**核心原则**：
> 跨仓库 session-log 按日期聚合提取，结果打印到对话供复制，不做持久化。

**标准动作序列**：
1. 提取用户消息中可能的日期信息（如 `日记 前天` → 前天），默认当天
2. 运行 `python scripts/aggregate-sessions.py [--date <日期>]`
3. 将输出张贴到对话中供用户复制
<!-- /@sync -->
