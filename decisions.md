...
| **决策** | **B. 聚焦 Excel 驱动流** |
| **理由** | 1. SKILL.md 已明确技能形态为「本地 Excel 驱动版」，当前 90% 价值来自这条流<br>2. API 流涉及外部认证、Digest 刷新、真实 BASE_URL 配置，维护成本高，且无活跃需求<br>3. Excel 驱动流已能满足「单店/多店分析 → Markdown 报告 → 飞书/钉钉推送」完整闭环<br>4. API 流代码骨架已删除（2026-06-25 清理），A→G 接口链知识保留在 lessons-learned.md 中 |
